...
describe("semrush.test", () => {
  it("succeeds and reports remaining API units", async () => {
    ...
    const r = await semrush.test({ database: "us", competitors: [] }, { api_key: "k1" }, f as unknown as typeof fetch);
  ...
  it("fails when SEMrush returns an error string", async () => {
    ...
    const r = await semrush.test({ database: "us", competitors: [] }, { api_key: "bad" }, f as unknown as typeof fetch);
