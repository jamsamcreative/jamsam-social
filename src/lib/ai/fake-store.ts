...
export type FakeStore = Store & { jobs: StoreJob[]; keywords: Keyword[]; imports: KeywordImport[]; created: { posts: unknown[]; articles: unknown[]; pins: unknown[] }; plans: Map<string, PlanMeta> };
...
export function fakeStore(over: Partial<Store> & Seed = {}): FakeStore {
  ...
  return Object.assign(base, overrides, { jobs, keywords, imports, created, plans });
}
