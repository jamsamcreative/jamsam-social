...
describe("fakePlanStore", () => {
  ...
  it("keeps an existing post_id when a re-sync upsert omits postId", async () => {
    const s = fakePlanStore();
    await s.upsertHistory("b1", [{ platform: "facebook", external_id: "1", published_at: "2026-01-01T00:00:00Z", caption: "", media: [], permalink: null, likes: 1, comments: 2, shares: 3, reach: null }], "p1");
    await s.upsertHistory("b1", [{ platform: "facebook", external_id: "1", published_at: "2026-01-01T00:00:00Z", caption: "", media: [], permalink: null, likes: 5, comments: 0, shares: 0, reach: 10 }]);
    expect(s.history).toHaveLength(1);
    expect(s.history[0].post_id).toBe("p1");
  });
});
